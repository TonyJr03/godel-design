...
| PPO-02D.2 | Ejecutada — `manual_required` por credencial DB CLI pendiente |

Este avance no cierra PPO-01, no aprueba `company-host`, no constituye
despliegue y no valida todavía Supabase administrado. PPO-02C.1 no constituye
...
valida sus propiedades, confirma CLI autenticada y proyecto enlazado accesible,
pero queda en `manual_required` porque falta una credencial de base de datos no
interactiva para consultar la historia remota de migraciones. PPO-02 general
permanece activa. El siguiente checkpoint previsto es reintentar PPO-02D.2
cuando esa credencial esté disponible de forma segura.
...
- PPO-02: definirá Dockerfile, Compose, Nginx, redes, healthchecks y criterios
  ...
  healthchecks locales y PPO-02D.2 queda ejecutada con resultado
  `manual_required`. Esto no implementa despliegue ni valida todavía Supabase
  administrado. El contrato activo vive en
