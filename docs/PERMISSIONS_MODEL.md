...
Para comentarios e historial de pedidos, las tablas oficiales normalizadas son `pedido_comentarios` y `pedido_historial`. Sus reglas siguen el acceso del pedido: `admin` y `supervisor` sobre cualquier pedido, y `trabajador` solo sobre pedidos asignados. Los comentarios de pedido son append-only inicialmente y el historial de pedido se escribe mediante flujos controlados como la RPC `actualizar_estado_pedido`.
