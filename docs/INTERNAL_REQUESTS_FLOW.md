...
La Fase 11.4 implementa comentarios internos en `/dashboard/solicitudes/[id]`. `admin` y `supervisor` pueden ver y agregar comentarios; el autor se toma del usuario autenticado en servidor mediante `solicitud_comentarios.author_id`. El formulario solo envía `solicitud_id` y `contenido`, no acepta autor ni fecha, y no hay edición ni eliminación.
