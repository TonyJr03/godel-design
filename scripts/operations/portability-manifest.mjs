...
function normalizedImages(lock) {
  return lock.images.map((image) => ({ logicalName: image.logicalName, canonicalRepository: image.canonicalRepository, manifestDigest: image.manifestDigest, configDigest: image.configDigest, platform: image.platform }));
}
...
function buildManifest(input, operationId) {
  ...
  return {
    ...
    contracts: { reconstructionManifestSchema: 1, imageLockSchema: 2, backupSchema: 3, secretGenerationStrategy: "EXACT_GENERATION_SNAPSHOTS", backupGenerationAlignment: "FAIL_CLOSED", implicitRollbackChain: "FORBIDDEN" },
  };
}
...
export function validateReconstructionManifest(manifest) {
  ...
  if (manifest.imageAuthority.format !== "godel-sh-portability-image-lock" || manifest.imageAuthority.schemaVersion !== 2 || !Number.isSafeInteger(manifest.imageAuthority.imageCount) || manifest.imageAuthority.imageCount < 1 || !Array.isArray(manifest.imageAuthority.images) || manifest.imageAuthority.images.length !== manifest.imageAuthority.imageCount) fail("IMAGE_AUTHORITY");
  sha256(manifest.imageAuthority.sha256, "IMAGE_AUTHORITY"); platform(manifest.imageAuthority.platform, "IMAGE_AUTHORITY");
  for (const image of manifest.imageAuthority.images) { exactKeys(image, ["logicalName", "canonicalRepository", "manifestDigest", "configDigest", "platform"], "IMAGE_INVENTORY"); string(image.logicalName, "IMAGE_INVENTORY", 64); string(image.canonicalRepository, "IMAGE_INVENTORY", 255); if (!/^sha256:[a-f0-9]{64}$/.test(image.manifestDigest) || !/^sha256:[a-f0-9]{64}$/.test(image.configDigest)) fail("IMAGE_INVENTORY"); platform(image.platform, "IMAGE_INVENTORY"); if (/godel-design-(app|nginx)/.test(image.logicalName + image.canonicalRepository)) fail("IMAGE_INVENTORY"); }
  ...
  if (manifest.contracts.reconstructionManifestSchema !== 1 || manifest.contracts.imageLockSchema !== 2 || manifest.contracts.backupSchema !== 3 || manifest.contracts.secretGenerationStrategy !== "EXACT_GENERATION_SNAPSHOTS" || manifest.contracts.backupGenerationAlignment !== "FAIL_CLOSED" || manifest.contracts.implicitRollbackChain !== "FORBIDDEN") fail("CONTRACTS");
