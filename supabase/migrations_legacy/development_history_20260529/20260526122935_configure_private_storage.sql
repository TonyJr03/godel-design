-- Configura la infraestructura privada de Supabase Storage para archivos de Godel Diseño.
-- Esta migración crea el bucket privado y las policies base de storage.objects.
-- No habilita subidas públicas anónimas.
...
is 'Valida lectura/listado de objetos del bucket privado godel-files según ruta, pedido, solicitud y rol interno.';
...
drop policy if exists godel_files_select_access on storage.objects;
...
drop policy if exists godel_files_insert_access on storage.objects;
...
drop policy if exists godel_files_update_manager on storage.objects;
...
drop policy if exists godel_files_delete_manager on storage.objects;
