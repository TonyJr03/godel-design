  ...
  return (
    <section className="rounded-lg border border-zinc-200 bg-white p-6 shadow-sm">
      ...
      <p className="mt-2 text-sm leading-6 text-zinc-600">
        Selecciona un usuario interno activo para agregarlo a este pedido.
      </p>

      {trabajadores.length === 0 ? (
        <p className="mt-4 rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-950">
          No hay personal interno activo disponible para asignar.
        </p>
      ) : (
        <form action={formAction} aria-busy={pending} className="mt-5 space-y-4">
          ...
          >
            {pending ? "Asignando..." : "Asignar personal"}
          </button>
        </form>
      )}
    </section>
