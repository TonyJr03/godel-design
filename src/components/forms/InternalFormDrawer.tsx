...
export type InternalFormDrawerProps = {
  isOpen: boolean;
  title: string;
  description?: string;
  children: ReactNode;
...
export function InternalFormDrawer({
  isOpen,
  title,
  description,
  children,
  ...
  return (
    <dialog
      ...
      className="fixed inset-0 m-0 hidden h-dvh max-h-dvh w-full max-w-none overflow-hidden border-0 bg-surface p-0 text-text-primary shadow-(--shadow-soft) backdrop:bg-text-primary/35 open:flex md:inset-y-0 md:left-auto md:right-0 md:ml-auto md:h-dvh md:w-[min(34rem,calc(100vw-2rem))] md:border-l md:border-border"
